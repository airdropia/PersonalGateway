...
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} GOARM=${TARGETVARIANT#v} go build \
	-ldflags="-s -w -X github.com/airdropia/pgw/internal/version.Version=${VERSION} -X github.com/airdropia/pgw/internal/version.Commit=${COMMIT} -X github.com/airdropia/pgw/internal/version.Date=${DATE}" \
